...
namespace WTFTwitch.HttpClient
{
    public class WTFTwitchHttpClient : IHttpCallHandler
    {
        private readonly ILogger<WTFTwitchHttpClient> _logger;
        ...
        public WTFTwitchHttpClient(ILogger<WTFTwitchHttpClient> logger = null)
        {
            _logger = logger;
            _http = new System.Net.Http.HttpClient(new TwitchHttpClientHandler(_logger));
        }
            ...
            string payload = null, ApiVersion api = ApiVersion.Helix, string clientId = null, string accessToken = null)
        {
            ...
            if (api == ApiVersion.Helix)
            {
                if (string.IsNullOrWhiteSpace(clientId) || string.IsNullOrWhiteSpace(accessToken))
                    throw new TwitchLib.Api.Core.Exceptions.InvalidCredentialException("A Client-Id and OAuth token is required to use the Twitch API.");
